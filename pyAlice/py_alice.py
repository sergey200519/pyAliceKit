...
from pyAlice.key_words import KeyWords
...
class PyAlice(Base):
    def __init__(self, params_alice: Union[dict, str], settings: ModuleType ) -> None:
    ...
    def init_key_word(self):
        try:
            if self.settings.TEXT_FOR_KEY_WORDS in self.params_alice["request"].keys():
                ...
                text_for_key_words = ""
            key_words = KeyWords(text=text_for_key_words, settings=self.settings, start_time=self.start_time).key_word()
    ...
    def get_params_for_alice(self, type: str = "json") -> Union[dict, str]:
        """
    ...
    def end_session_alice(self) -> None:
        self.end_session = True
        self.add_log("session_end", color="purple", start_time=self.start_time)

    def __reset(self) -> None:
        self.new, self.end_session= False, False
        # str
        self.result_message, self.came_message = "", ""
        # dict
        self.intents, self.storage, self.events, self.logs, self.more_data_message = {}, {}, {}, {}, {}
        # list
        self.buttons, self.alice_buttons, self.key_words = [], [], []
